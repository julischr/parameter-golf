...
class Hyperparameters():
    ...
    tied_embed_lr = float(os.environ.get('TIED_EMBED_LR', 0.003))
    tied_embed_init_std = float(os.environ.get('TIED_EMBED_INIT_STD', 0.005))
    matrix_lr = float(os.environ.get('MATRIX_LR', 0.02))
    scalar_lr = float(os.environ.get('SCALAR_LR', 0.002))
...
class BitLinear(nn.Linear):
    def forward(self, x: Tensor) -> Tensor:
        ...
        # 1. Gewichte quantisieren (Global, wie bisher)
        w_scale = w_float.abs().mean().clamp_min(1e-5)
        w_quant = torch.round(w_float / w_scale).clamp(-1.0, 1.0)
        w_final = (w_quant * w_scale) + w_float - w_float.detach()
        
        # === 2. DER ECHTE BUG-FIX: PER-TOKEN QUANTISIERUNG ===
        # Das Maximum wird jetzt pro Token (dim=-1) berechnet, um Ausreißer abzufangen!
        x_max = x_float.abs().max(dim=-1, keepdim=True)[0].clamp_min(1e-5)
        x_scale = 127.0 / x_max
        
        x_quant = torch.round(x_float * x_scale).clamp(-128.0, 127.0) / x_scale
        x_final = x_quant + x_float - x_float.detach()
        
        # 3. Sicherheitsnetz gegen Hardware-Rundungsfehler
        w_final = torch.nan_to_num(w_final)
        x_final = torch.nan_to_num(x_final)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if HAS_FA3:
            # Für die H100 (Flash Attention 3 erwartet [Batch, Seq, Heads, Dim])
            y = flash_attn_3_func(q, k, v, causal=True)
        else:
            # Fallback für deine A40
            q_pt = q.transpose(1, 2)
            k_pt = k.transpose(1, 2)
            v_pt = v.transpose(1, 2)
            y = F.scaled_dot_product_attention(
                q_pt, k_pt, v_pt, is_causal=True, enable_gqa=(self.num_kv_heads != self.num_heads)
            )
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        # --- PARALLEL RESIDUALS (Platz 1 Upgrade) ---
        # Attention und MLP lesen BEIDE denselben Input (x_in) gleichzeitig
        attn_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor)
        mlp_out = self.mlp(self.mlp_norm(x_in) * self.ln_scale_factor)
        
        # Beide Ergebnisse werden am Ende zusammen auf den Stream addiert
...
def pack_model_weights(state_dict: dict[str, Tensor]) -> tuple[dict[str, Tensor], dict[str, object]]:
    ...
    for name, tensor in state_dict.items():
        ...
        # 1. Unsere 1.58-Bit Matrizen (Attention & MLP) -> extrem winzig gerundet
        if any(k in name for k in ["c_q", "c_k", "c_v", "proj", "fc"]) and "tok_emb" not in name:
            scale = t.abs().mean().clamp_min(1e-8)
            t_quant = torch.round(t / scale).clamp(-1.0, 1.0).to(torch.int8)
            result[name + ".weight"] = t_quant
            result[name + ".scale"] = scale.to(torch.float16)
            meta[name] = "ternary (int8)"
        
        # 2. Die Embeddings (Vokabular) -> Simples 8-Bit Min-Max statt fehlerhaftem GPTQ
        elif "tok_emb" in name:
            scale = (t.abs().max() / 127.0).clamp_min(1e-8)
            t_quant = torch.round(t / scale).clamp(-127.0, 127.0).to(torch.int8)
            result[name + ".weight"] = t_quant
            result[name + ".scale"] = scale.to(torch.float16)
            meta[name] = "minmax (int8)"
            
        # 3. Der Rest (Norms, Biases) -> Unkomprimiertes float16
        else:
            result[name] = t.to(torch.float16) if t.is_floating_point() else t
            meta[name] = "passthrough (float16)"
            
        short = re.sub(r'\.\d+$', '', re.sub(r'blocks\.\d+', 'blocks', name))
        categories[meta[name]].add(short)

    log("Quantized weights (Efficient 1.58-Bit Mode):")
...
def unpack_model_weights(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info == "passthrough (float16)":
            t = result[name]
            if t.dtype == torch.float16 and orig_dtype in (torch.float32, torch.bfloat16):
                t = t.to(orig_dtype)
            out[name] = t
            
        elif info in ["ternary (int8)", "minmax (int8)"]:
            w_quant = result[name + ".weight"].to(orig_dtype)
            scale = result[name + ".scale"].to(orig_dtype)
            out[name] = w_quant * scale
...
def serialize(h: Hyperparameters, base_model: torch.nn.Module, code: str) -> tuple[int, int]:
    ...
    log("Packing weights efficiently (bypassing GPTQ)...")
    quant_result, quant_meta = pack_model_weights(sd_cpu)
...
def deserialize(h: Hyperparameters, device: torch.device) -> GPT:
    ...
    deq_state = unpack_model_weights(quant_state["w"], quant_state["m"], sd_cpu)
